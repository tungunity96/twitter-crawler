  ...
  function writeToCSVFile() {
    ...
    if (users && users.length > 0) {
      users.forEach((user, index) => {
        fileContent += `${index},${user.name},${user.user},${user.page},${user.tweet},${user.timestamp},${user.tweetContent},${user.detectedTags}\r\n`;
      });
        ...
        if (index !== 0) {
          ...
          contentSpanTags.each(function (index) {
            ...
            if (
              ...
              detectedTags +=
                " " + escape(spanContent.trim().replace("\n", ""));
          });
